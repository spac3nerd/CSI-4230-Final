...
//generate a usable list of items from what Plaid gave us
router.post("/categories/getall", async function(req, res) {
    let primaryCatMap = {}; //used to keep track of already generated items
    let secondaryCatMap = {}; //map to filter out thrird+ tier categories
    let primaryCategories = [];
    let secondaryCategories = [];

    for (let k = 0; k < catDefinition.length; k++) {
        if (primaryCatMap.hasOwnProperty(catDefinition[k].hierarchy[0])) {
            if (catDefinition[k].hierarchy[1] !== null) {
                if (!secondaryCatMap.hasOwnProperty(catDefinition[k].hierarchy[1])) {
                    secondaryCategories[primaryCatMap[catDefinition[k].hierarchy[0]]].push(catDefinition[k].hierarchy[1]);
                    secondaryCatMap[catDefinition[k].hierarchy[1]] = true;
                }
            }
        }
        else {
            primaryCatMap[catDefinition[k].hierarchy[0]] = primaryCategories.length;
            primaryCategories.push(catDefinition[k].hierarchy[0]);
